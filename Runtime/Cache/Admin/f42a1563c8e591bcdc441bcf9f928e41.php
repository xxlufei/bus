...
<div class="main">
	...
	<p class="short-input ue-clear">
    	<label>密码：</label>
        <input name="password" type="text" placeholder="密码" /> 注：留空不修改密码
    </p>
    <p class="short-input ue-clear">
    	<label>姓名：</label>
    ...
    <div class="short-input select ue-clear">
        ...
        <select name="dept_id">
            ...
            <?php if(is_array($rst)): $i = 0; $__LIST__ = $rst;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                <option value="<?php echo ($vo["id"]); ?>" <?php if($data["dept_id"] == $vo["id"] ): ?>selected="selected"<?php endif; ?>><?php echo (str_repeat("&emsp;",$vo["level"]*2)); echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
    </div>
	<p class="short-input ue-clear">
    	<label>性别：</label>
