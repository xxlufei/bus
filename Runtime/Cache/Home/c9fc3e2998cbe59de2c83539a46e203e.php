...
<head>
    ...
    <link rel="shortcut icon" href="favicon.ico"> <link href="/Public/Hplus/css/bootstrap.min14ed.css?v=3.3.6" rel="stylesheet">
            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-3" style="margin-top: -8px">
                                    <select id="province" class="form-control m-b" name="account" style="display: inline;">
                                        <?php if(is_array($province)): $i = 0; $__LIST__ = $province;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$prov): $mod = ($i % 2 );++$i;?><option value="<?php echo ($prov["province_id"]); ?>" <?php if($prov["province_id"] == $current_prov_id): ?>selected<?php endif; ?>><?php echo ($prov["province_name"]); ?></option>　<?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                    ...
                    <div class="ibox-content" id="city">
                        <?php if(is_array($citys)): $i = 0; $__LIST__ = $citys;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$city): $mod = ($i % 2 );++$i; if($city["city_id"] == $current_city_id): ?><a href="javascript:;" class="text-danger"><b><i><?php echo ($city["city_name"]); ?></i></b></a>　　
                                <?php else: ?>
                                <a href="javascript:;" onclick="change_city(<?php echo ($city["city_id"]); ?>)"><?php echo ($city["city_name"]); ?></a>　　<?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    var current_city_id = <?php echo ($current_city_id); ?>;
    var city_obj = eval(<?php echo ($json_citys); ?>);
    $('#province').on('change', function () {
        $.get("<?php echo U('city');?>",{current_prov_id:$('#province').val()}, function(data){
    ...
    $('#line').on('click', function () {
        $.get("<?php echo U('line');?>",{current_city_id:current_city_id, line_name:$('#line_name').val()}, function(data){
            var list = eval(data);//解析json
            if (list.length == 0) {
    ...
    $('#station').on('click', function () {
        $.get("<?php echo U('station');?>",{station_name:$('#station_name').val()}, function(data){
            var list = eval(data);//解析json
            if (list.length == 0) {
                alert('没有此线路,请联系管理员添加');
                return;
    ...
    $('#stationA_to_stationB').on('click', function () {
        ...
        $.get("<?php echo U('stationA_to_stationB');?>",{station_name_A:$('#station_name_A').val(), station_name_B:$('#station_name_B').val()}, function(data){
            if (data == 1) {
